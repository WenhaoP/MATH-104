...
\begin{document}
	...
	\section{Some Topological Concepts in Metric Spaces}
		\begin{dfn}
		Let $S$ be a set, and suppose $d$ is a function $d: X\times X\rightarrow[0,\infty]$ defined for all pairs $(x,y)$ of elements from $S$ satisfying
		\begin{enumerate}
			\item $d(x,x)=0$ for all $x\in S$ and $d(x,y)>0$ for distinct $x,y\in S$. (Positive Definiteness)
		...
		\begin{rem}
		The positive definiteness can be also expressed as $\forall x,y\in S\ d(x,y)\geq 0$ and $d(x,y)=0\iff x=y$. The distance function cannot be $+\infty$.
		\end{rem}
		
		\begin{dfn}
		\begin{itemize}
			\item[]
			\item A sequence $(s_n)$ in a metric space $(S,d)$ converges to $s$ in $S$ if $\lim_{n\rightarrow\infty}d(s_n,s)=0$.
			\item A sequence $(s_n)$ in $S$ is a \emph{Cauchy sequence} if for each $\epsilon>0$ there exists an $N$ such that
			\begin{displaymath}
				m,n>N\Rightarrow d(s_m,s_n)<\epsilon.
			\end{displaymath}
			\item $(S,d)$ is said to be \emph{complete} if every Cauchy sequence in $S$ converges to some element in $S$.
		\end{itemize}
		\end{dfn}
		...
		\end{dfn}
		\begin{rem}
			The relationship between $E$ and $X$ may affect whether a point in $E$ is interior to $E$. For example, for $E=[0,1]\subset[-1,2]=X$, $0$ is not interior to $[0,1]$. However if $E=[0,1]\subset[0,1]=X$, then $0$ is interior to $0$ since there is not point on the left of $0$.
		\end{rem}
		
		\begin{dfn}[Open Set]
			A set $E\subseteq X$ is \emph{open} if every point $x\in E$ is an interior point of $E$.	
		\end{dfn}
		The set $E$ is \emph{open} in $X$ if every point in $E$ is interior to $E$, i.e., if $E=E^\circ$.
		...
		\begin{dfn}[Limit Point]
			...
			$E^\prime:=$ set of all limit points of $E$.
		\end{dfn}
		\begin{rem}
			In other words, $x$ is a limit point of $E$ if for any (small) radius $r$, there is some element of $E$ which sits in $B_r(x)$, other than $x$ itself.
		\end{rem}
		...
		\end{dfn}
		\begin{eg}
			In $\R$, $E$ is bounded if there exists $M$ such that $|x|\leq M$ for all $x\in E$.
		\end{eg}\smallskip
		\begin{dfn}[Closure]
		...
		\begin{dfn}[Closed Set]
			Let $(S,d)$ be a metric space. A subset $E$ of $S$ is \emph{closed} if its complement $S\backslash E$ is an open set.\smallskip
			
			The intersection of any collection of closed sets is closed. The \emph{closure} $E^-$ of a set $E$ is the intersection of all closed sets containing $E$.\smallskip
			
			The \emph{boundary} of $E$ is the set $E^-\backslash E^\circ$; points in this set are called \emph{boundary points}.\smallskip
		\end{dfn}
		\begin{rem}
			Equivalence of two definitions: TODO
		\end{rem}
		...
		\begin{itemize}
			\item[]
			\item A sequence $(\textbf{x}^{(n)})$ in $\mathbb{R}^k$ converges if and only if for each $j=1,2\dots,k$, the sequence $(x_j^{(n)})$ converges in $\mathbb{R}$.
			\item A sequence $(\textbf{x}^{(n)})$ in $\mathbb{R}^k$ is a Cauchy sequence if and only if each sequence $(x_j^{(n)})$ is a Cauchy sequence in $\mathbb{R}$.
		\end{itemize}	
		\end{lem}
		\begin{proof}
		First assertion: TODO.
