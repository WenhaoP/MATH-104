...
\begin{document}
	...
	\chapter{Continuity}
		...
		`	
			...
			\end{proof}
			\begin{rem}
				Be careful that if $f$ is \emph{only continuous} (not uniformly) on $\overline{E}$, then (ii) fails.
			\end{rem}
			\newpage
	...
	\chapter{Sequences and Series of Functions}
		...
		\section{More on Uniform Convergence}
			...
			\end{proof}
			\begin{cor}
				$\sum_{k=0}^{\infty}g_k$ satisfies the uniform Cauchy criterion if and only if it converges.
			\end{cor}
	...
	\chapter{Differentiation}
		...
		\section{The Mean Value Theorem}
			\begin{lem}
			Let $f$ be defined on an open interval $I$ containing $x$. If $f$ attains its maximum (or minimum) at $x$ and $f$ is differentiable at $x$, then $f^\prime(x)=0$.
			\end{lem}
			\begin{proof}
